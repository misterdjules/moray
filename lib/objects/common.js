...
function visitFilter(filter, schema, func) {
    ...
    case 'or':
        filter.filters.forEach(function (subFilter) {
            visitFilter(subFilter, schema, func);
        });
        break;

    case 'not':
        visitFilter(filter.filter, schema, func);
        break;
    ...
    default:
        if (filter.value !== undefined && filter.value !== null) {
            func(schema, filter.attribute, filter.value, filter);
        }
        break;
    }
}
...
function convertFilterValueToIndexType(schema, attribute, value, filter) {
    assert.object(schema, 'schema');
    assert.object(filter, 'filter');
    ...
    if (filter.type === 'ext') {
        ...
        case '2.5.13.2':
        case 'caseIgnoreMatch':
            _matchCaseInsensitive(filter);
            break;
        case '2.5.13.4':
        case 'caseIgnoreSubstringsMatch':
            _matchCaseInsensitiveSubstr(filter);
            break;
        default:
    ...
    if (schema.index[attribute] !== undefined) {
        switch (schema.index[attribute].type) {
        case 'ip':
        case '[ip]':
            _matchTypeIP(filter);
            break;
        case 'subnet':
        case '[subnet]':
            _matchTypeSubnet(filter);
            break;
        case 'boolean':
        case '[boolean]':
            filter.value = /^true$/i.test(value);
            break;
        case 'number':
        case '[number]':
...
function decorateFilter(req, cb) {
    ...
    try {
        visitFilter(req.filter, req.bucket, convertFilterValueToIndexType);
        /*
...
function loadBucket(req, cb) {
    ...
    var requiredBucketVersion = req.opts.requiredBucketVersion;

    if (!noCache && (b = req.bucketCache.get(ckey))) {
        req.bucket = b;

        if (requiredBucketVersion === undefined ||
            b.options.version >= requiredBucketVersion) {
            log.debug({
                bucket: b
...
function rowToObject(bucket, row, ignore) {
    ...
    var rowVer = row._rver;
    var bucketVer = bucket.options.version;
    var rowAtLatestVersion = rowVer >= bucketVer;
    ...
    //
    // - if the PG column is null, then do not consider that data as valid
    // unless that data is known as being up to date. This means that if the
    // bucket cache for a given moray instance is _not_ up to date, it's
    // possible that a row will be considered to be at the latest version, and
    // thus some properties of objects could be removed when they should have
    // not been removed. This could result in e.g findobjects queries using
    // search filters returning erroneous filters. If users require accurate
    // results when searching objects they need to pass a value for the
    // "requiredBucketVersion" option to ensure that the bucket cache is
    // refreshed if the cache is stale.
    //
    ...
    bucket._indexKeys.forEach(function (key) {
        ...
        if (v === undefined || v === null) {
            if (obj.value[k] !== undefined && rowAtLatestVersion)
                delete obj.value[k];
        } else {
            if (obj.value[k] === undefined || !Array.isArray(obj.value[k])) {
                obj.value[k] = v;
            }
        }
    });
